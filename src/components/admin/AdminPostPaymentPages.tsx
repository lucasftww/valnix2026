              ...
              <div className="bg-card border border-border/50 rounded-xl p-6 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={page.title} onChange={(e) => updatePage(page.id, { title: e.target.value })} className="bg-muted/50 border-border/50" />
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">Subtítulo</Label>
                    <Input value={page.subtitle || ""} onChange={(e) => updatePage(page.id, { subtitle: e.target.value })} className="bg-muted/50 border-border/50" />
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">Badge</Label>
                    <Input value={page.badge_text || ""} onChange={(e) => updatePage(page.id, { badge_text: e.target.value })} className="bg-muted/50 border-border/50" />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      className="w-full h-9 rounded-md bg-muted/50 border border-border/50 px-3 text-sm text-foreground"
                    >
                  ...
                  <div>
                    ...
                    <Input type="number" step="0.01" value={page.price} onChange={(e) => updatePage(page.id, { price: parseFloat(e.target.value) || 0 })} className="bg-muted/50 border-border/50" />
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">Preço Original (riscado)</Label>
                    <Input type="number" step="0.01" value={page.original_price || ""} onChange={(e) => updatePage(page.id, { original_price: parseFloat(e.target.value) || null })} className="bg-muted/50 border-border/50" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs text-muted-foreground">Texto botão aceitar</Label>
                    <Input value={page.button_accept_text} onChange={(e) => updatePage(page.id, { button_accept_text: e.target.value })} className="bg-muted/50 border-border/50" />
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">Texto botão pular</Label>
                    <Input value={page.button_skip_text} onChange={(e) => updatePage(page.id, { button_skip_text: e.target.value })} className="bg-muted/50 border-border/50" />
                  </div>
                  <div className="md:col-span-2">
                    <Label className="text-xs text-muted-foreground">Próxima rota (next_route)</Label>
                    <Input value={page.next_route || ""} onChange={(e) => updatePage(page.id, { next_route: e.target.value })} placeholder="/protecao-total" className="bg-muted/50 border-border/50" />
                  </div>
                </div>
                ...
                <div className="bg-muted/30 border border-border/50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <Label className="text-[10px] text-muted-foreground/70">utm_source</Label>
                      <Input placeholder="ex: instagram" value={getUtm(page.addon_type).source} onChange={(e) => setUtm(page.addon_type, "source", e.target.value)} className="bg-card border-border/50 text-xs h-8" />
                    </div>
                    <div>
                      <Label className="text-[10px] text-muted-foreground/70">utm_medium</Label>
                      <Input placeholder="ex: cpc" value={getUtm(page.addon_type).medium} onChange={(e) => setUtm(page.addon_type, "medium", e.target.value)} className="bg-card border-border/50 text-xs h-8" />
                    </div>
                    <div>
                      <Label className="text-[10px] text-muted-foreground/70">utm_campaign</Label>
                      <Input placeholder="ex: promo_fev" value={getUtm(page.addon_type).campaign} onChange={(e) => setUtm(page.addon_type, "campaign", e.target.value)} className="bg-card border-border/50 text-xs h-8" />
                    </div>
                  </div>
