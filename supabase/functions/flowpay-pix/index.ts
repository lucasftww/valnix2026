  ...
  try {
    ...
    if (req.method === 'POST' && action === 'webhook') {
      ...
      await Promise.allSettled([
        ...
        (async () => { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'webhook', event_id: purchaseEventId, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('meta-capi', { event_name: 'Purchase', event_id: purchaseEventId, order_id: orderId, value: orderValue, currency: 'BRL', content_name: productNamesList, email: customerEmail, phone: customerPhone || undefined, first_name: nameParts[0] || undefined, last_name: nameParts.slice(1).join(' ') || undefined, external_id: userId, fbc: orderFields?.fbc?.stringValue, fbp: orderFields?.fbp?.stringValue, event_source_url: orderFields?.event_source_url?.stringValue || undefined }); console.log(`📡 [Meta] CAPI Purchase sent — event_id=${purchaseEventId} (webhook)`); } else { console.log(`⏭️ [Meta] CAPI Purchase skipped — already sent for order ${orderId}`); } })(),
        ...
        try {
          ...
          if (orderDoc) {
            ...
            if (orderFields?.payment_status?.stringValue !== 'paid') {
              ...
              try { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'polling', event_id: pollingEventId, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('meta-capi', { event_name: 'Purchase', event_id: pollingEventId, order_id: orderId, value: Number(orderValue), currency: 'BRL', content_name: pollingProductNames, email: fbEmail, phone: fbPhone || undefined, first_name: nameParts[0] || undefined, last_name: nameParts.slice(1).join(' ') || undefined, external_id: fbUserId, fbc: orderFields?.fbc?.stringValue, fbp: orderFields?.fbp?.stringValue, event_source_url: orderFields?.event_source_url?.stringValue || undefined }); console.log(`📡 [Meta] CAPI Purchase sent — event_id=${pollingEventId} (polling)`); } else { console.log(`⏭️ [Meta] CAPI Purchase skipped — already sent for order ${orderId}`); } } catch {}
