  ...
  try {
    ...
    if (req.method === 'POST' && action === 'webhook') {
      ...
      await Promise.allSettled([
        addFirestoreDocWithId('analytics_events', `card_purchase_${orderId}`, { event_name: 'Purchase', event_time: new Date().toISOString(), user_id: userId || null, value: orderValue, currency: 'BRL', order_id: orderId, content_name: productNamesList, source: 'card_webhook' }),
        (async () => { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'card_webhook', event_id: `purchase_${orderId}`, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('meta-capi', { event_name: 'Purchase', event_id: `purchase_${orderId}`, order_id: orderId, value: orderValue, currency: 'BRL', content_name: productNamesList, email: customerEmail, phone: customerPhone || undefined, first_name: nameParts[0] || undefined, last_name: nameParts.slice(1).join(' ') || undefined, external_id: userId, fbc: orderFields?.fbc?.stringValue, fbp: orderFields?.fbp?.stringValue, event_source_url: orderFields?.event_source_url?.stringValue || undefined }); } })(),
    ...
    if (action === 'confirm' && req.method === 'POST') {
      ...
      await Promise.allSettled([
        addFirestoreDoc('analytics_events', { event_name: 'Purchase', event_time: new Date().toISOString(), user_id: userId || null, value: orderValue, currency: 'BRL', order_id: orderId, page_url: 'https://www.valnix.com.br/card-callback', content_name: productNamesList }),
        (async () => { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'card_confirm', event_id: `purchase_${orderId}`, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('meta-capi', { event_name: 'Purchase', event_id: `purchase_${orderId}`, order_id: orderId, value: orderValue, currency: 'BRL', content_name: productNamesList, email: customerEmail, phone: customerPhone || undefined, first_name: nameParts[0] || undefined, last_name: nameParts.slice(1).join(' ') || undefined, external_id: userId, fbc: orderFields.fbc?.stringValue, fbp: orderFields.fbp?.stringValue, event_source_url: orderFields.event_source_url?.stringValue || undefined }); } })(),
        invokeEdgeFunction('utmify-event', { order_id: orderId, event_type: 'Purchase', value: orderValue, customer_name: customerName, customer_email: customerEmail, customer_phone: customerPhone || undefined, product_name: productNamesList, utm_source: orderFields.utm_source?.stringValue, utm_medium: orderFields.utm_medium?.stringValue, utm_campaign: orderFields.utm_campaign?.stringValue, utm_content: orderFields.utm_content?.stringValue, utm_term: orderFields.utm_term?.stringValue }),
      ]);
