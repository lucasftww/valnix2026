  ...
  try {
    ...
    if (req.method === 'POST' && action === 'webhook') {
      ...
      await Promise.allSettled([
        ...
        (async () => { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'card_webhook', event_id: cardEventId, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('server-relay', buildMetaCapiPurchasePayload(cardEventId, orderId, orderValue, items, orderFields, customerEmail, customerPhone, customerName, userId)); console.log(`📡 [Meta] CAPI Purchase sent — event_id=${cardEventId} (card_webhook)`); } else { console.log(`⏭️ [Meta] CAPI Purchase skipped — already sent for order ${orderId}`); } })(),
        ...
        if (orderResults?.[0]?.document) {
          ...
          if (orderFields?.payment_status?.stringValue !== 'paid') {
            ...
            await Promise.allSettled([
              ...
              (async () => { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'card_auto_verify', event_id: pollingEventId, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('server-relay', buildMetaCapiPurchasePayload(pollingEventId, orderId, orderValue, items2, orderFields, customerEmail, customerPhone, customerName, userId)); console.log(`📡 [Meta] CAPI Purchase sent — event_id=${pollingEventId} (card_auto_verify)`); } })(),
    ...
    if (action === 'confirm' && req.method === 'POST') {
      ...
      await Promise.allSettled([
        ...
        (async () => { const r = await addFirestoreDocWithId('meta_purchase_events', orderId, { sent_at: new Date().toISOString(), source: 'card_confirm', event_id: confirmEventId, created_at: new Date().toISOString() }); if (r) { await invokeEdgeFunction('server-relay', buildMetaCapiPurchasePayload(confirmEventId, orderId, orderValue, items3, orderFields, customerEmail, customerPhone, customerName, userId)); console.log(`📡 [Meta] CAPI Purchase sent — event_id=${confirmEventId} (card_confirm)`); } else { console.log(`⏭️ [Meta] CAPI Purchase skipped — already sent for order ${orderId}`); } })(),
